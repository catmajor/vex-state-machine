...
class CAMERA_CONTROL_STATE(State):
    ...
    def act(self):
        ...
        if (len(close)>0):
            ...
            y = close[0].centerY - CAMERA_Y_DIV_2 + 50
            self.last_x = x
        elif (len(far)>0):
        ...
        effort_y = 30*self.gain_function(error_y)
...
class DRIVE_STATE(State):
    class Direction:
        FORWARD = 1
        BACKWARD = -1
    def __init__(self, direction, target_angle, stop_handler: Handler):
...
ROBOT_CLOSE = CustomHandler(lambda: ULTRASONIC_G.distance(DistanceUnits.CM)<16)
...
class FRUIT_APPROACH_STATE(DRIVE_STATE):
    def __init__(self):
        DRIVE_STATE.__init__(self, DRIVE_STATE.Direction.FORWARD, 0, LIGHT_LEFT_ACTIVE)
